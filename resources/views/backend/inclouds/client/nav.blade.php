        ...
        <div class="navbar-brand app-brand demo d-none d-xl-flex py-0 me-4">
            ...
            @if (auth()->user()->hasRole('client'))
                @foreach (auth()->user()->companies as $company)
                    <a href="{{ route('client.company', $company->id) }}" class="d-flex align-items-center ms-3 btn btn-sm btn-outline-light waves-effect fw-bold rounded-pill pe-2">
                        <span class="fs-5 me-2">✨</span>
                        {{ $company->name }}
                        
                        <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center ms-2">
                            @foreach ($company->users as $cuser)
                                <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top"
                                class="avatar avatar-xs pull-up @if ($cuser->id == $company->user_id ) avatar-online @endif"
                                title="{{ $cuser->name }}">
                                    <span class="avatar-initial rounded-circle bg-dark text-white">{{ substr($cuser->name,0,2); }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </a>
        ...
        <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
            ...
            <ul class="navbar-nav flex-row align-items-center ms-auto">
                {{-- <li class="nav-item me-2 me-xl-0">
                    <a class="nav-link dropdown-toggle hide-arrow" href="javascript:void(0);" id="shepherd-example" >
                        <i class="ti ti-question-mark fis rounded-circle me-1 fs-3"></i>
                    </a>
                </li> --}}
                <!-- Language -->
                    ...
                    <ul class="dropdown-menu dropdown-menu-end py-0">
                                ...
                                <li class="list-group-item list-group-item-action dropdown-notifications-item">
                                    <div class="d-flex">
                                            ...
                                            <div class="avatar">
                                                <div class="avatar text-center">
                                                    @if ( $notification->data['category'] == 'support-message' )
                                                        <i class="fs-3 text-success ti ti-message-dots"></i>
                                                    @elseif ( $notification->data['category'] == 'new-delegation' )
                                                        <i class="fs-3 text-danger ti ti-flag-3"></i>
                                                    @else
                                        ...
                                        <div class="flex-grow-1">
                                            ...
                                            @if ($notification->data['category'] == 'new-payment' || $notification->data['category'] == 'new-project-data' || $notification->data['category'] == 'new-delegation')
                                                <a href="{{ route('single.project', $notification->data['link']) }}" class="mb-0"><strong>{!! $notification->data['title'] !!} </strong> <br> {!! $notification->data['description'] !!}</a>
                                            @else
                        ...
                        {{-- <li class="dropdown-menu-footer border-top">
                            <a
                                ...
                                {{ __('general.View all notifications') }}
                            </a>
                        </li> --}}
                    </ul>
                </li>
